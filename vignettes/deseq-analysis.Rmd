...
labs <- 
  ...
  left_join(colData(lf_hyp_bay_se), by = c("label" = "id"), copy = TRUE)
...
df <- 
  ...
  left_join(colData(lf_hyp_bay_se), by = c("sample_1" = "id"), copy = TRUE) %>% 
  mutate(
...
interaction <- 
  ...
  left_join(as_tibble(rowData(lf_hyp_bay_se), rownames = "entrez_id"), by = "entrez_id") %>% 
